...
import type { CardsStats } from '@/lib/stats';
...
// Same layout/components as StatsBar (Components tab) - scoped to one Cards
// tab. See getCardsStats in lib/stats.ts for why "Total Icons"/"Hardcoded"
// aren't repeated here.
export function CardsStatsBar({ stats, basePath }: { stats: CardsStats; basePath: string }) {
  return (
    <div className="statsbar">
      <div className="stats-row">
        <StatChip label="Total Cards" value={stats.totalCards} />
      ...
      {stats.topUsed.length > 0 && (
        <div className="top-used__card">
          ...
          >
            Top {stats.topUsed.length} most-used cards
          </Typography>
          <div className="top-used__labels">
            {stats.topUsed.map((c, i) => (
              <Link key={c.slug} href={`${basePath}#${c.slug}`} className="top-used__labelLink">
